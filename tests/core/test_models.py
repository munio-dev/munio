...
class TestCompositeExpressionSyntaxError:
    """Cover _validate_expression_ast SyntaxError branch (lines 327-329)."""

    @pytest.mark.parametrize(
        ("expression", "desc"),
        [
            ("x +", "trailing-operator"),
            ("if x else y", "ternary-not-expression"),
            ("x ==", "incomplete-comparison"),
            ("(x + ", "unclosed-paren"),
        ],
        ids=["trailing-op", "ternary", "incomplete-cmp", "unclosed-paren"],
    )
    def test_composite_invalid_expression_syntax_rejected(self, expression: str, desc: str) -> None:
        """COMPOSITE with unparseable expression raises validation error."""
        with pytest.raises(ValidationError, match="Invalid expression syntax"):
            ConstraintCheck(
                type=CheckType.COMPOSITE,
                field="*",
                variables={"x": CompositeVariable(field="amount")},
                expression=expression,
            )


class TestDenylistMatchRegexEmptyValue:
    """Cover empty string rejection in match=regex values (lines 504-505)."""

    @pytest.mark.parametrize(
        "check_type",
        [CheckType.DENYLIST, CheckType.ALLOWLIST],
        ids=["denylist", "allowlist"],
    )
    def test_match_regex_empty_value_rejected(self, check_type: CheckType) -> None:
        """match=regex with empty string in values is rejected."""
        with pytest.raises(ValidationError, match="empty strings"):
            ConstraintCheck(
                type=check_type,
                field="url",
                values=["valid_pattern", ""],
                match=MatchMode.REGEX,
            )


class TestDenylistSanitization:
    """Cover value sanitization branch (line 526) and post-sanitization empty check."""

    def test_control_char_values_are_sanitized(self) -> None:
        """Values with control characters are sanitized at load time."""
        check = ConstraintCheck(
            type=CheckType.DENYLIST,
            field="input",
            values=["hello\x00world"],
        )
        # Control chars should be stripped by _sanitize_string
        assert all("\x00" not in v for v in check.values)

    def test_control_char_only_value_rejected_after_sanitization(self) -> None:
        """Value that becomes empty after sanitization is rejected."""
        with pytest.raises(ValidationError, match=r"empty strings.*after sanitization"):
            ConstraintCheck(
                type=CheckType.DENYLIST,
                field="input",
                values=["\x00\x01\x02"],
...
class TestThresholdNanInf:
    """Cover NaN/Inf rejection in threshold min/max (lines 549-550)."""

    @pytest.mark.parametrize(
        ("kwargs", "desc"),
        [
            ({"min": float("nan")}, "min-nan"),
            ({"max": float("nan")}, "max-nan"),
            ({"min": float("inf")}, "min-inf"),
            ({"max": float("-inf")}, "max-neg-inf"),
            ({"min": float("inf"), "max": float("inf")}, "both-inf"),
        ],
        ids=["min-nan", "max-nan", "min-inf", "max-neg-inf", "both-inf"],
    )
    def test_threshold_nan_inf_rejected(self, kwargs: dict, desc: str) -> None:
        """Threshold check rejects NaN and Inf for min/max."""
...
class TestRegexPolynomialReDoS:
    """Cover polynomial ReDoS pattern rejection (lines 574-575)."""

    @pytest.mark.parametrize(
        ("check_type", "pattern", "desc"),
        [
            (
                CheckType.REGEX_DENY,
                r"[a-z]+[a-z]+[a-z]+[a-z]+[a-z]+",
            ...
            (
                CheckType.REGEX_ALLOW,
                r"\w+\w+\w+\w+\w+",
                "allow-5-consecutive-word",
            ),
            (
                CheckType.REGEX_DENY,
                r"[0-9]+[0-9]+[0-9]+[0-9]+[0-9]+",
                "deny-5-digit-classes",
            ),
        ],
        ids=["deny-5-consecutive", "allow-5-consecutive-word", "deny-5-digit-classes"],
    )
    def test_polynomial_redos_rejected(
        self, check_type: CheckType, pattern: str, desc: str
    ) -> None:
        """Regex patterns with 5+ consecutive quantified atoms are rejected."""
        with pytest.raises(ValidationError, match="polynomial ReDoS"):
            ConstraintCheck(
                type=check_type,
                field="input",
                patterns=[pattern],
    ...
    def test_polynomial_redos_in_match_regex_values_rejected(self) -> None:
        """match=regex with polynomial ReDoS pattern in values is rejected."""
        with pytest.raises(ValidationError, match="polynomial ReDoS"):
            ConstraintCheck(
                type=CheckType.DENYLIST,
                field="url",
