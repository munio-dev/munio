...
def _check_permissive_approval(server: ServerConfig) -> list[Finding]:
    """SC_011: Detect MCP server configs with safety controls disabled by default."""
    findings: list[Finding] = []

    # Env vars that disable safety controls
    approval_env_vars: dict[str, frozenset[str]] = {
        "APPROVAL_MODE": frozenset(
            {"off", "disable", "disabled", "false", "no", "0", "permissive"}
        ),
        "ASK": frozenset({"off", "disable", "disabled", "false", "no", "0", "permissive"}),
        "SAFETY_MODE": frozenset({"off", "disable", "disabled", "false", "no", "0", "permissive"}),
        "CONFIRMATION": frozenset({"off", "disable", "disabled", "false", "no", "0", "permissive"}),
        "AUTO_APPROVE": frozenset({"true", "yes", "1", "all"}),
    }

    if server.env:
        for var_name, var_value in server.env.items():
            upper = var_name.strip().upper()
            if upper in approval_env_vars:
                if var_value.strip().lower() in approval_env_vars[upper]:
                    findings.append(
                        Finding(
                            id="SC_011",
                            layer=Layer.L0_CONFIG,
                            ...
                            tool_name=server.name,
                            message=(
                                f"Safety control disabled via env '{var_name}={var_value}'"
    ...
    # Dangerous args that disable safety
    unsafe_args: frozenset[str] = frozenset(
        {
            "--no-approval",
            "--no-confirm",
            "--no-safety",
        ...
        if arg in unsafe_args:
            findings.append(
                Finding(
                    ...
                    tool_name=server.name,
                    message=(
                        f"Safety control disabled via argument '{arg}'"
                        " -- permissive approval default"
                    ),
                    attack_type=AttackType.SAFETY_TAMPERING,
                    cwe="CWE-269",
...
def _check_ws_origin_validation(server: ServerConfig) -> list[Finding]:
    """SC_012: Detect WebSocket MCP servers without origin validation."""
    findings: list[Finding] = []
    ...
    # Check for origin validation config
    origin_env_vars = frozenset({"CORS_ORIGINS", "ALLOWED_ORIGINS", "ORIGIN_WHITELIST"})
    origin_args = frozenset({"--origin", "--cors"})

    has_origin_config = False
    if server.env:
        for var_name in server.env:
            if var_name.strip().upper() in origin_env_vars:
                has_origin_config = True
                break

    if not has_origin_config:
        for arg in server.args:
            if arg in origin_args or arg.startswith(("--origin=", "--cors=")):
                has_origin_config = True
                break
    ...
    if not has_origin_config:
        findings.append(
            Finding(
                ...
                tool_name=server.name,
                message=(
                    f"WebSocket server '{url}' has no origin validation"
...
def _check_host_mode_exec(server: ServerConfig) -> list[Finding]:
    """SC_013: Detect MCP server configs running outside sandbox on host."""
    findings: list[Finding] = []

    host_env_checks: dict[str, frozenset[str]] = {
        "EXEC_HOST": frozenset({"gateway"}),
        "SANDBOX": frozenset({"off", "false", "disabled", "none"}),
        "ISOLATION": frozenset({"off", "false", "disabled", "none"}),
    }

    if server.env:
        for var_name, var_value in server.env.items():
            upper = var_name.strip().upper()
            if upper in host_env_checks:
                if var_value.strip().lower() in host_env_checks[upper]:
                    findings.append(
                        Finding(
                            id="SC_013",
                            layer=Layer.L0_CONFIG,
                            ...
                            tool_name=server.name,
                            message=(
                                f"Host-mode execution via env '{var_name}={var_value}'"
    ...
    host_args: frozenset[str] = frozenset(
        {
            "--host-mode",
            "--no-sandbox",
            "--privileged",
        ...
        if arg in host_args:
            findings.append(
                Finding(
                    ...
                    severity=FindingSeverity.HIGH,
                    tool_name=server.name,
                    message=(f"Host-mode execution via argument '{arg}' -- no sandbox isolation"),
                    attack_type=AttackType.SAFETY_TAMPERING,
                    cwe="CWE-269",
                    location=f"server:{server.name}",
                    confidence=0.9,
...
_SERVER_CHECKS = [
    ...
    _check_permissive_approval,
    _check_ws_origin_validation,
    _check_host_mode_exec,
]


def _check_server(name: str, server: ServerConfig | dict[str, Any]) -> list[Finding]:
    """Run all server checks against a config (dict or ServerConfig)."""
