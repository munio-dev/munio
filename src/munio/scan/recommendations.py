...
_RECOMMENDATIONS: dict[str, Recommendation] = {
    ...
    # SCT: Safety Control Tampering
    "SC_011": Recommendation(
        short="Review MCP server config for safety-disabling defaults. "
        "Ensure approval mode requires explicit user confirmation.",
        auto_fixable=False,
    ),
    "SC_012": Recommendation(
        short="Enable WebSocket origin validation. Use wss:// instead of ws:// "
        "for non-localhost connections. Set ALLOWED_ORIGINS.",
        auto_fixable=False,
    ),
    "SC_013": Recommendation(
        short="Do not run MCP server tools in host mode. Use container/sandbox "
        "isolation for code execution.",
        auto_fixable=False,
    ),
    # L3_020: Safety tool detection
    "L3_020": Recommendation(
        short="Tool exposes safety control modification. Ensure pre-execution "
        "verification is enforced for all calls to this tool.",
        auto_fixable=False,
    ),
}
