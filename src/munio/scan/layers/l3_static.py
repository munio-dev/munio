...
# ── L3_020: Safety control tool detection ────────────────────────────────

_SAFETY_TOOL_SEGMENTS: frozenset[str] = frozenset(
    {
        "approval",
        "approvals",
        "safety",
...
# Enum values that indicate disabling/weakening safety controls
_SAFETY_DISABLE_ENUMS: frozenset[str] = frozenset(
    {
        "off",
        "disable",
        "disabled",
...
# Parameter names that boost confidence for safety control modification
_SAFETY_BOOST_PARAMS: frozenset[str] = frozenset(
    {
        "mode",
        "level",
        "enabled",
...
def _is_safety_tool(tool_name_normalized: str) -> bool:
    """Check if tool name suggests it modifies safety controls."""
    return any(seg in tool_name_normalized for seg in _SAFETY_TOOL_SEGMENTS)


def _safety_confidence_boost(
    properties: dict[str, Any],
) -> float:
    """Return confidence boost based on parameter analysis.

    Base confidence: 0.70
    Boost to 0.85 if any param has enum with disable-like values.
    Boost to 0.90 if param name is mode/level/enabled/status.
    """
    confidence = 0.70
    for param_name, param_def in properties.items():
        if not isinstance(param_def, dict):
            continue
        pname = param_name.lower().replace("-", "_").replace(".", "_")
        # Check param name boost
        if pname in _SAFETY_BOOST_PARAMS:
            confidence = max(confidence, 0.90)
        # Check enum values for disable-like patterns
        enum_vals = param_def.get("enum")
        if isinstance(enum_vals, list):
            for val in enum_vals:
                if isinstance(val, str) and val.lower() in _SAFETY_DISABLE_ENUMS:
...
class L3StaticAnalyzer:
    ...
    def analyze_tool(self, tool: ToolDefinition | dict[str, Any]) -> list[Finding]:
        """Analyze a single tool definition. Accepts ToolDefinition or raw dict."""
        ...
        return self._analyze_tool(tool)
    ...
    def _analyze_tool(self, tool: ToolDefinition) -> list[Finding]:
        ...
        # ── L3_020: Safety control tool detection ──────────────────────
        if _is_safety_tool(tool_normalized):
            confidence = _safety_confidence_boost(properties)
            findings.append(
                self._finding(
                    "L3_020",
                    tool.name,
                    ...
                    (
                        f"Tool '{tool.name}' appears to modify safety controls "
                        f"(approvals, guardrails, security settings)"
                    ),
                    attack_type=AttackType.SAFETY_TAMPERING,
                    cwe="CWE-269",
                    confidence=confidence,
                    description=(
                        f"Tool '{tool.name}' has a name suggesting it can "
                        f"modify safety-critical configurations such as "
                        f"approval modes, guardrails, sandbox settings, or "
                        f"access controls. An attacker could invoke this tool "
                        f"to weaken or disable safety controls before "
                        f"executing a destructive action."
                    ),
                )
            )
