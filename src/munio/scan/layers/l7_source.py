...
import logging
import re  # noqa: TC003 — used at runtime (re.compile)
...
class L7SourceAnalyzer:
    ...
    def _parse_all_files(self) -> dict[Path, object]:
        ...
        return parsed  # type: ignore[return-value,unused-ignore]
            ...
            if node.type == "import_statement":
                ...
                for c in node.children:
                    if c.type == "dotted_name":
                        ...
                        if alias_node and name_node:
                            imports[self._text(alias_node, source)] = self._text(name_node, source)
                ...
                for c in node.children:
                    if c.type == "dotted_name" and c != module_node:
                        ...
                        name_node = c.child_by_field_name("name")
                        if alias_node and name_node:
                            imports[self._text(alias_node, source)] = module
                        elif name_node:
                            imports[self._text(name_node, source)] = module
        return imports
    ...
    @classmethod
    def _get_sql_re(cls) -> re.Pattern[str]:
    ...
    @staticmethod
    def _walk_descendants(node: Node) -> Iterator[Node]:
        """Yield all descendant nodes."""
        cursor = node.walk()
        visited = False
        while True:
            if not visited and cursor.node is not None:
                yield cursor.node
