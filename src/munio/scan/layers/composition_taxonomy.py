...
_KNOWN_TOOLS: dict[str, tuple[ToolRole, frozenset[Capability]]] = {
    ...
    # safety control modification (P|S: reads current config + modifies it)
    "exec.approvals.set": (ToolRole.P | ToolRole.S, frozenset({Capability.SAFETY_CONFIG})),
    "config.patch": (ToolRole.P | ToolRole.S, frozenset({Capability.SAFETY_CONFIG})),
    "config.set": (ToolRole.P | ToolRole.S, frozenset({Capability.SAFETY_CONFIG})),
    "security.set": (ToolRole.P | ToolRole.S, frozenset({Capability.SAFETY_CONFIG})),
    "permissions.update": (ToolRole.P | ToolRole.S, frozenset({Capability.SAFETY_CONFIG})),
    "guardrails.disable": (ToolRole.P | ToolRole.S, frozenset({Capability.SAFETY_CONFIG})),
    "sandbox.config": (ToolRole.P | ToolRole.S, frozenset({Capability.SAFETY_CONFIG})),
}
...
_TOXIC_FLOW_RULES: tuple[ToxicFlowRule, ...] = (
    ...
    # Safety Control Tampering (SCT) toxic flows
    ToxicFlowRule(
        source_caps=frozenset({Capability.FETCH_UNTRUSTED}),
        sink_caps=frozenset({Capability.SAFETY_CONFIG}),
        risk="CRITICAL",
        description="Untrusted external data can disable safety controls before exploitation",
        cwe="CWE-269",
    ),
    ToxicFlowRule(
        source_caps=frozenset({Capability.SAFETY_CONFIG}),
        sink_caps=frozenset({Capability.CODE_EXEC}),
        risk="CRITICAL",
        description="Safety controls can be weakened before code execution",
        cwe="CWE-269",
    ),
    ToxicFlowRule(
        source_caps=frozenset({Capability.CREDENTIAL_READ}),
        sink_caps=frozenset({Capability.SAFETY_CONFIG}),
        risk="HIGH",
        description="Stolen credentials can be used to modify safety configurations",
        cwe="CWE-863",
    ),
)
...
_CAPABILITY_NAME_KEYWORDS: dict[Capability, frozenset[str]] = {
    ...
    Capability.SAFETY_CONFIG: frozenset(
        {
            "approval",
            "approvals",
            "safety",
...
def classify_tool(tool: ToolDefinition) -> tuple[ToolRole, frozenset[Capability]]:
    ...
    if role != ToolRole.NONE and not caps:
        rv = role.value  # avoid mypy Flag Literal narrowing
        inferred: set[Capability] = set()
        if rv & ToolRole.P.value:
            inferred.add(_DEFAULT_SOURCE_CAP)
        if rv & ToolRole.U.value:
            inferred.add(_DEFAULT_UNTRUSTED_CAP)
        if rv & ToolRole.S.value:
